			...
			<div class="right">
				<div class="form">
					...
					<div class="msg-footer">
						<textarea 
							v-model="inputRedis[selectInfo.id]" 
							maxlength="500" 
...
export default{
	...
	methods:{
		...
		// 添加处理Enter键的方法
		handleEnterKey(e) {
			// 如果按下的是Enter键且没有同时按下Shift键
			if (e.key === 'Enter' && !e.shiftKey) {
				e.preventDefault(); // 只在非Shift+Enter时阻止默认行为
				this.sendMsg();
			}
			// 如果是Shift+Enter，不做任何处理，让浏览器默认行为（插入换行）生效
		}
	},
	async mounted() {
		...
		// 默认有2个消息
		this.chatInfo["-1"] = this.packMsg("-1","在线直聘智能机","sys",new Date().toLocaleString(),require("@/assets/image/user/Cat.png")
			,[{"type":"sys","text":"\
				<div class='sys-msg'>\
					<h2>登录操作通知</h2>\
					<div class='describe'>["+this.me.nickname+"]欢迎回来在线招聘系统</div>\
					<div class='info'>\
						<div>\
							<span>设备/平台</span>\
...
<style lang="scss" scoped>
	...
	.msg-content .right {
		...
		.form {
			...
			.content {
				flex: 0.95; /* 从flex: 1改为0.7，减小聊天记录区域 */
				overflow: auto;
				padding: 20px;
		...
		.msg-footer {
			...
			textarea {
				...
				&:focus {
					border-color: rgb(128,185,242);
					box-shadow: 0 1px 5px rgba(128,185,242,0.2);
				}
			}
			.send {
				position: absolute;
				right: 20px;
				bottom: 20px; /* 调整发送按钮位置 */

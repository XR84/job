...
public class UserServiceImpl extends ServiceImpl<UserMapper, UserDO> implements UserService {
    ...
    public UserDO createUser(RegisterDTO dto) {
        ...
        if (exist) {
            throw new ForbiddenException(10071,"已经有用户使用了该名称，请重新输入新的用户名");
        }
    ...
    public UserDO changeUserPassword(ChangePasswordDTO dto) {
        ...
        if (!valid) {
            throw new FailedException(10011,"更新密码失败");
        }
        return user;
    }
    ...
    public CreatedVO forgotPwd(JSONObject param) {
        ...
        if(data.getString("username").isEmpty() || data.getString("password").isEmpty()|| data.getString("ver_code").isEmpty()){
        ...
        if(user == null || redisService.getMailCode(data.getString("ver_code"))==null){
...
//    }
    ...
    private void checkGroupsExist(List<Integer> ids) {
        for (Integer id : ids) {
            if (!groupService.checkGroupExistById(id)) {
                throw new NotFoundException(10023,"分组不存在，无法新建用户");
            }
        }
    }
    ...
    private void checkGroupsValid(List<Integer> ids) {
        ...
        if (anyMatch) {
            throw new ForbiddenException(10073,"root分组不可添加用户");
        }
    }
}

...
public class PositionController {
    ...
    public CreatedVO create(@RequestBody @Validated CreateOrUpdatePositionDTO validator) {
        ...
        return new CreatedVO(2100,"职位发布成功，请等待管理员审核");
    }
    ...
    public UpdatedVO updatePosition(@PathVariable @Positive(message = "{id.positive}") Integer id, @RequestBody @Validated CreateOrUpdatePositionDTO validator) {
        ...
        if (positionDO == null) {
            throw  new NotFoundException(41000,"职位不存在");
        }
        positionService.updatePosition(positionDO, validator);
        return new UpdatedVO(2200,"更新职位信息成功，请等待管理员审核");
    }
    ...
    public UpdatedVO update(@PathVariable @Positive(message = "{id.positive}") Integer id, @RequestParam Integer state) {
        ...
        return new UpdatedVO(4000,"更新职位状态成功");
    }
    ...
    public UpdatedVO updateHits(@PathVariable @Positive(message = "{id.positive}") Integer id) {
        ...
        return new UpdatedVO(4100,"更新职位浏览量成功");
    }
    ...
    public DeletedVO delete(@PathVariable @Positive(message = "{id.positive}") Integer id) {
        ...
        return new DeletedVO(2000,"职位下架成功");
    }
    ...
    public List<PositionRecommendVO> recommend(@RequestParam Integer userId) {
        // 1. 先查询用户的简历ID
        ResumeDO resume = resumeService.getOne(new QueryWrapper<ResumeDO>()
                .eq("user_id", userId)
                .isNull("delete_time"));
        if (resume == null) {
            throw new NotFoundException(30000, "未找到用户简历");
        }

        // 2. 查询用户的教育经历中的专业
        List<EducationDO> educations = educationService.list(new QueryWrapper<EducationDO>()
                .eq("resume_id", resume.getId())
                .orderByDesc("diploma")); // 按学历降序排序，取最高学历的专业

        if (educations.isEmpty()) {
            throw new NotFoundException(30000, "未找到用户教育经历");
        }

        // 3. 使用自定义SQL查询推荐职位
        return positionMapper.selectRecommendPositions(educations.get(0).getMajor());
    }
